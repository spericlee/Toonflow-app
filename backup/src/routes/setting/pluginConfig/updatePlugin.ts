...
export default router.post(
  ...
  async (req, res) => {
    const { base64Data, id, link } = req.body;
    ...
    try {
      ...
      if (id != manifestContent.id) return res.status(400).send(error("id与插件不匹配 无法更新"));
      ...
      if (!fs.existsSync(pluginDir)) {
        fs.rmSync(tempDir, { recursive: true, force: true });
        return res.status(400).send(error(`插件 ${pluginId} 不存在，请先安装`));
      }
      ...
      res.status(200).send(success(manifestContent, "插件更新成功"));
    } catch (e: any) {
      // 出错时清理临时目录
      if (fs.existsSync(tempDir)) {
        fs.rmSync(tempDir, { recursive: true, force: true });
      }
      return res.status(400).send(error(e.message || "插件更新失败"));
    }
  },
);
