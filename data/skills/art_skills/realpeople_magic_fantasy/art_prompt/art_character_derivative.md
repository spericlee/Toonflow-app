# 人物衍生资产生成 · 约束手册

---

## 一、叠加原则

1. **面容不变** — 叠加后五官必须与底模完全一致，禁止面容偏移
2. **姿态不变** — 保持底模自然站立姿态，禁止任何姿态/动作/体态变化
3. **逐层可控** — 每层独立描述，便于按层替换（换装不换妆）
4. **风格统一** — 所有服化元素服从同一美学体系
5. **质感不降** — 叠加后质感标准不低于底模
6. **纯服化范畴** — 仅叠加妆容/发型/服饰/配饰，禁止引入道具、场景、环境、动作

---

## 二、叠加层级

| 层级 | 内容 | 说明 |
|---|---|---|
| L0 | 底模 | 基础形象底模，不修改 |
| L1 | 妆容（决策层） | 先分析用户线索，再决策「基础妆 / 轻妆 / 正式妆」强度 |
| L2 | 发型造型 | 编发/束发/披散 + 发饰 |
| L3 | 内搭/中衣 | 替换基础内衬 |
| L4 | 外衣/主服 | 魔法长袍/铠甲/礼服等 |
| L5 | 配饰 | 头饰/耳饰/项饰/腰饰/手饰 |

> **范畴边界**：人物衍生资产仅包含 L0–L5 层级（服化妆造），不包含道具（法杖/剑/弓/书/药剂等手持物）、场景环境（室内/室外/天气等）、姿态动作（行走/回眸/施法/举手等）。这些属于其他资产类型的范畴。

---

## 三、妆容约束（L1）

### 底模到衍生妆造策略（关键）

> 角色底模虽为素颜，但衍生资产默认进入妆造流程。系统应根据用户提供的线索分析妆造需求，并在基础妆、轻妆、正式妆之间决策强度，而不是保持素颜。

### L1 线索分析与妆容决策

| 步骤 | 处理内容 | 决策结果 |
|---|---|---|
| S1 | 提取用户线索：面部状态词、情绪词、强度词 | 形成妆容需求摘要 |
| S2 | 过滤非妆容线索：道具/场景/动作/姿态词不作为上妆依据 | 防止误判 |
| S3 | 匹配妆容风格矩阵并给出强度档 | 基础妆 / 轻妆 / 正式妆 |
| S4 | 生成最终 L1 提示词 | 只输出结论，不输出分析过程 |

### 线索到妆容映射（执行口径）

| 线索类型 | 典型线索 | L1 决策 |
|---|---|---|
| 无明显面部强调线索 | 仅服饰/发型变化，未强调情绪与状态 | 基础妆 |
| 轻微面部线索 | 轻柔、浅笑、睫毛轻颤、气色微提 | 轻妆（极淡） |
| 明确病弱线索 | 面色苍白、唇色极淡、眼下微红 | 病弱妆（轻妆） |
| 明确正式仪式线索 | 盛装、典礼、魔法觉醒、华贵亮相 | 正式妆（受控） |

> 判定原则：所有衍生资产都要有妆造；先看面部线索决定强度与风格，道具、场景、姿态变化不得单独抬高妆容强度。

### 女性妆容风格矩阵

| 风格 | 适用场景 | 核心提示词 |
|---|---|---|
| 清透素妆 | 日常、初遇、居所 | 妆容清透、自然裸妆、素妆清颜 |
| 冷艳霜妆 | 对峙、权力、法师对决 | 妆容冷艳、眉眼锋利、薄唇冷冽 |
| 柔魅桃妆 | 浪漫、暧昧、心动 | 桃花妆、眼尾微红、唇色水润 |
| 病弱妆 | 受伤、虚弱、诅咒 | 面色苍白、唇色极淡、眼下微红 |
| 华贵盛妆 | 加冕、典礼、魔法觉醒 | 浓妆华美、朱唇凤眼 |

### 通用底肤（所有妆容共享）

| 项目 | 约束 | 提示词 |
|---|---|---|
| 质感 | 水光肌、自然透亮 | 水光肌、奶油瓷肌、luminous skin |
| 白度 | 冷白皮/瓷白肌、通透不惨白 | 牛奶肌、porcelain white skin |
| 内透光 | 从内向外柔光感 | 内透光感、皮肤通透发光 |
| 禁止 | 哑光/死白/蜡感/油光/过曝 | — |

### 基础妆细化（默认档）

| 项目 | 约束 | 提示词 |
|---|---|---|
| 眉部 | 顺着底模眉形轻修，不改变眉型 | 自然修眉、眉形干净 |
| 眼部 | 极淡眼部修饰，强调清透与有神 | 眼部清透、极淡内眼线 |
| 面颊 | 极淡气色提亮，不可明显堆色 | 面颊气色自然、微弱提气色 |
| 唇部 | 裸粉或浅粉润色，保持克制 | 唇色自然润泽、浅粉唇色 |
| 整体 | 看得出有妆造，但妆感非常轻 | 基础妆、伪素颜妆感、自然精修 |

### 分部位（以清透素妆为例）

| 部位 | 约束 | 提示词 |
|---|---|---|
| 底妆 | 轻薄通透、水光微光泽 | 底妆轻薄、水光奶油肌 |
| 眉妆 | 自然眉形、浅棕淡扫 | 自然眉形、淡扫蛾眉 |
| 眼妆 | 极淡眼影、内眼线、睫毛纤长 | 眼妆清透、睫毛纤长 |
| 腮红 | 极淡薄粉、苹果肌微扫 | 腮红极淡、薄粉微醺 |
| 唇妆 | 水润浅粉、微光泽 | 唇色水润浅粉 |

### 男性妆容

| 项目 | 约束 | 提示词 |
|---|---|---|
| 底肤 | 水光奶油肌、白皙透亮/健康麦色、清爽自然 | 水光肌、奶油肌、luminous skin |
| 原则 | 伪素颜——看着没化妆但皮肤极好 | 伪素颜、天生好皮 |
| 眉毛 | 自然浓眉、不画眉 | 剑眉自然、眉形英挺 |
| 唇色 | 自然血色、微润 | 唇色自然、血色感 |

---

## 四、发型造型约束（L2）

### 女性造型类型

| 造型 | 描述 | 适用 | 提示词 |
|---|---|---|---|
| 半挽编发 | 发顶编辫+后方垂发 | 日常、出行 | 半挽编发、发辫精致 |
| 高髻飞挑 | 高髻飞挑、飘逸优雅 | 精灵、亮相 | 高髻飞挑、优雅高髻 |
| 侧披卷发 | 一侧披散微卷、慵懒 | 私密、暧昧 | 侧披卷发、慵懒微卷 |
| 双辫编发 | 双辫对称、少女感 | 年轻角色 | 双辫编发、少女双辫 |
| 全散发 | 长发全散、配简单发饰 | 受伤、落魄 | 长发散落、青丝如瀑 |
| 高束马尾 | 高束干练 | 行动、战斗 | 束发高马尾、干练利落 |

### 女性发饰

| 项目 | 约束 | 提示词 |
|---|---|---|
| 风格 | 精致主义，与服饰配套 | 精致发饰、华美精致 |
| 材质 | 金属 + 宝石 + 珍珠 + 丝带 | 银质发饰、宝石镶嵌、丝带飘逸 |
| 工艺 | 大师工艺、超精细 | 大师工艺、精雕细琢 |

### 男性造型类型

| 造型 | 适用 | 提示词 |
|---|---|---|
| 半束散发 | 日常、文人法师 | 半束长发、丝带束发 |
| 全冠高束 | 正式、王室 | 全冠高束、王冠束发 |
| 散发披肩 | 私密、受伤 | 散发披肩、长发如墨 |
| 战束马尾 | 战斗、骑士 | 高束战发、马尾利落 |

---

## 五、服饰约束（L3+L4）

### 女性服饰矩阵

| 风格 | 款式 | 适用 | 提示词 |
|---|---|---|---|
| 飘逸法袍 | 多层长袍、丝绒/绸缎 | 日常、魔法 | 长袍曳地、多层衣衫、衣料飘逸 |
| 端庄礼服 | 束腰长裙、丝绒披肩 | 宴会、朝堂 | 束腰长裙、端庄华美 |
| 轻便劲装 | 皮甲/紧身上衣+裙裤 | 行动、战斗 | 皮甲劲装、轻便利落 |
| 寝衣 | 薄纱内衫、素色 | 室内、夜间 | 素色寝衣、宽松舒适 |
| 盛装礼裙 | 层叠裙摆、宝石装饰 | 加冕、典礼 | 层叠礼裙、宝石点缀 |

### 女性服饰通用约束

| 项目 | 约束 | 提示词 |
|---|---|---|
| 主色 | 白色/月华银/暮光紫为默认 | 白色精致衣袍、素衣如雪 |
| 材质 | 厚实质感 + 刺绣 + 珠光面料 | 衣料厚实飘逸、珠光刺绣 |
| 质感 | 纹理必须超清晰 | 衣服质感清晰、纹理超清晰 |
| 肩部 | 披肩/斗篷/肩饰 | 斗篷华美、肩头有装饰 |
| 层次 | 多层叠穿、层次分明 | 多层叠穿、层次分明 |

### 男性服饰矩阵

| 风格 | 适用 | 提示词 |
|---|---|---|
| 法师雅装 | 日常、书卷 | 宽袖长袍、素色法袍 |
| 骑士劲装 | 战斗、训练 | 锁甲劲装、金属战靴 |
| 暗夜斗篷 | 出场、潜行 | 墨色斗篷、披风猎猎 |
| 常服便装 | 休闲、私密 | 素色常服、简约便装 |
| 礼服朝服 | 王室、典礼 | 正式礼服、华贵长袍 |

---

## 六、配饰约束（L5）

### 女性配饰

| 类型 | 约束 | 提示词 |
|---|---|---|
| 头饰 | 精致主义、不单薄 | 精致头饰、宝石点缀 |
| 耳饰 | 垂坠流苏/宝石耳坠 | 流苏耳环、宝石垂坠 |
| 项饰 | 项链/吊坠/魔法宝石 | 宝石项链、魔法吊坠 |
| 腰饰 | 腰带/腰链/流苏 | 编织腰带、腰间流苏 |
| 手饰 | 手镯/手链/戒指 | 银质手镯、精致戒指 |

### 男性配饰

| 类型 | 约束 | 提示词 |
|---|---|---|
| 王冠/头饰 | 金银/宝石、精致 | 王冠束发、银质发饰 |
| 腰封 | 宽皮带/金属链 | 宽腰封、质感分明 |
| 项链/徽章 | 家族徽章、魔法吊坠 | 胸前徽章、吊坠垂挂 |
| 兵器 | 佩剑/匕首/法杖（可选） | 长剑在侧、法杖手边 |

---

## 七、服化组合速查

| 场景 | 妆容 | 发型 | 服饰 | 配饰 |
|---|---|---|---|---|
| 日常居所 | 清透素妆 | 半挽编发 | 飘逸法袍 | 中等 |
| 初次相遇 | 清透素妆 | 半挽/高髻 | 飘逸法袍 | 中偏多 |
| 浪漫互动 | 柔魅桃妆 | 半挽/侧披 | 法袍/轻便 | 中等 |
| 正式亮相 | 冷艳霜妆 | 高髻飞挑 | 端庄礼服 | 极繁 |
| 夜间密谈 | 清透/桃妆 | 全散/侧披 | 寝衣 | 极简 |
| 受伤落魄 | 病弱妆 | 全散（乱） | 破损常服 | 极简/无 |
| 加冕典礼 | 华贵盛妆 | 高髻飞挑 | 盛装礼裙 | 极繁 |
| 战斗行动 | 素妆（极淡） | 束发马尾 | 轻便劲装 | 简 |

---

> **🔍 未覆盖场景推断规则**
>
> 当用户描述的场景/情境不在上表时，根据本风格核心基因自行推断：
>
> | 推断维度 | 真人写实魔幻基因 |
> |---|---|
> | 妆容强度 | 默认清透素妆（水光肌+发丝写实）；权力/对峙→冷艳霜妆；心动/暧昧→柔魅桃妆；受伤/虚弱→病弱妆；典礼/觉醒→华贵盛妆 |
> | 发型 | 日常/居所→半挽编发；精灵/亮相→高髻飞挑；私密/暧昧→侧披卷发；受伤落魄→全散发；行动→束发马尾；发丝必须根根分明 |
> | 服饰 | 真人写实质感优先；日常→法袍/飘逸长裙；正式→束腰礼服；战斗→皮甲劲装；主色默认白/月华银/暮光紫；纹理必须超清晰 |
> | 配饰繁度 | 写实工艺精致主义（大师工艺精雕细琢）；日常→中等；正式→极繁（宝石项链+精致发饰+流苏腰带）；战斗→简；受伤→极简/无 |
> | 质感基准 | 真人写实摄影锚定；水光奶油瓷肌+发丝细节始终保持；禁止 3D 渲染/CG 感 |

## 八、四视图设定图规范

> 衍生服化叠加后仍需输出四视图设定图，确保服化妆造在各角度的一致性。

### 视图定义

| 位置 | 视图 | 角度 | 景别 | 要求 | 提示词 |
|---|---|---|---|---|---|
| 左一 | 人像特写 | 正面平视 | 面部至锁骨 | 面部占60%+，五官/妆容清晰 | portrait closeup、face detail、makeup detail |
| 左二 | 正视图 | 正面 0° | 全身立像 | 面对镜头、服饰正面全貌 | front view、height mark |
| 右二 | 侧视图 | 右侧 90° | 全身立像 | 纯侧面轮廓、服饰侧面层次 | side view、profile、height mark |
| 右一 | 后视图 | 后方 180° | 全身立像 | 后脑发饰/背部服饰/发尾清晰 | back view、rear view、height mark |

### 画面规范

| 项目 | 约束 |
|---|---|
| 布局 | 同一画面从左至右并排四视图 |
| 背景 | 纯净中性灰 #E8E8E8 |
| 站姿 | 自然站立、双脚平行微分、双臂自然下垂或微展（**禁止任何姿态变化**） |
| 表情 | 符合妆容风格的微表情（如清透素妆→淡然、桃妆→浅笑），仅限面部微表情，不涉及肢体动作 |
| 光线 | 均匀柔光，前方主光 + 双侧补光，无硬阴影 |
| 一致性 | 四视图的面容/妆容/发型/发饰/服饰/配饰完全一致 |
| 画面比例 | 建议 4:1 或 3:1 |

---

## 九、提示词模板

### 输出格式约束

| 项目 | 约束 |
|---|---|
| 输出内容 | **仅输出提示词文本**，不输出任何其他内容 |
| 禁止输出 | 速查表、分层构建方案、视觉约束表、禁止事项表、衍生方案、输出建议、核心要素表等一切非提示词内容 |
| 禁止场景 | 人物衍生资产**不包含场景/环境描述**，不输出任何场景/环境/天气/背景叙事内容（场景属于场景资产范畴） |
| 禁止道具 | **不包含任何道具交互**，不输出法杖/剑/弓/书/药剂/灯笼等手持物或交互物（道具属于道具资产范畴） |
| 禁止姿态变化 | **不改变底模姿态**，不输出行走/回眸/施法/举手/侧身/奔跑等任何动作或体态变化，保持自然站立 |
| 格式 | 直接输出可用的提示词代码块，无需标题、表格、解释、方案对比 |

### 完整服化叠加（四视图）

```
以角色基础形象图为底图，img2img叠加服化妆造，
魔幻{性别}角色四视图设定图，真人写实摄影，魔幻写实纪实，强对比度，极致细节，8K，超保真
character design sheet，character turnaround，
保持基础形象面容不变，{整体气质}，
【L1·妆容】根据用户线索决策：{基础妆/轻妆/正式妆}；使用 {妆容风格}，水光奶油瓷肌，{眉妆}，{眼妆}，{唇妆}，
【L2·发型】{造型类型}，发丝根根分明，{发饰描述}，
【L3+L4·服饰】{主色}{款式}，{材质}，{装饰工艺}，衣服质感清晰，纹理超清晰，
【L5·配饰】{头饰}，{耳饰}，{项饰}，{腰饰}，
同一画面左至右并排：人像特写+正视图+侧视图+后视图，
自然站立，纯净中性灰背景，均匀柔光，无硬阴影，
四视图一致性，面容细腻渲染，发丝细腻渲染，纹理细节超清晰
图中不要有任何文字
```

---

## 十、约束规则

### 必守

| 编号 | 规则 |
|---|---|
| R1 | 叠加后面容必须与底模一致 |
| R2 | 服饰必须用「衣服质感清晰 + 纹理超清晰」 |
| R3 | 女性配饰必须「精致主义 + 大师工艺」 |
| R4 | 妆容/发型/服饰/配饰风格统一 |
| R5 | 必须输出四视图设定图（人像特写+正视图+侧视图+后视图） |
| R6 | 必须指定「纯净中性灰背景」 |
| R7 | 必须指定「四视图一致性」 |
| R8 | **仅输出提示词**——禁止输出速查表/分层方案/视觉约束/禁止事项/衍生方案/输出建议等任何非提示词内容 |
| R9 | **禁止包含场景描述**——人物衍生资产不涉及场景/环境/天气/背景叙事，场景属于独立资产类型 |
| R10 | **禁止道具交互**——不包含任何手持物/交互物（法杖/剑/弓等），道具属于独立资产类型 |
| R11 | **姿态保持不变**——必须保持底模自然站立姿态，禁止任何动作/体态/姿势变化 |
| R12 | **L1 必须先分析再决策**——先解析用户面部线索，再确定基础妆/轻妆/正式妆 |
| R13 | **所有衍生资产均需妆造**——正常情况不保持素颜，至少使用基础妆 |
| R14 | **上妆强度受控**——即使上妆也需克制，不得出现现代浓妆/夸张彩妆效果 |
| R15 | **道具/场景/动作不作强度升级依据**——仅凭道具，环境，动作等信息不得把基础妆抬高为更强妆容 |

### 严禁

| 编号 | 严禁 |
|---|---|
| X1 | 叠加后面容偏移 |
| X2 | 配饰过于简单/现代化（女性） |
| X3 | 妆容/服饰风格互相冲突 |
| X4 | 复杂场景背景（必须纯灰底） |
| X5 | 四视图间服化妆造不一致 |
| X6 | 输出提示词以外的任何内容（表格/方案/建议/解释/变体等） |
| X7 | 在人物衍生资产中加入场景描述（森林/城堡/雨景/街道/天气等环境元素） |
| X8 | 输出「核心要素速查」「分层构建方案」「视觉约束」「禁止事项」「衍生方案」等章节 |
| X9 | 加入任何道具交互（手持法杖/剑/弓/书/药剂等物品） |
| X10 | 改变底模姿态（行走/回眸/施法/举手/侧身/奔跑/低头/仰望等动作描述） |
| X11 | 加入表情与姿态联动描述（如「侧身45°嘴角浅弯施法」等叙事性描写） |
| X12 | 未分析用户线索就直接套用固定妆容 |
| X13 | 错误保持素颜，导致衍生资产缺少应有妆造 |
| X14 | 仅因道具/场景/动作词而误把妆容升级，导致妆造强度决策错误 |
